---
name: anime-fx
description: Manage anime effects — change themes, swap sounds, toggle on/off, adjust volume, configure TTS narration
user-invocable: true
...
# Anime FX Plugin Manager

Config file: `~/Coding/anime-fx-plugin/config.json`
Env file: `~/Coding/anime-fx-plugin/.env`
Sounds dir: `~/Coding/anime-fx-plugin/sounds/`

## Commands

### `/anime-fx` or `/anime-fx status`
...
Show the user how to preview: `afplay ~/Coding/anime-fx-plugin/sounds/custom/<file>.wav`

### `/anime-fx volume <0.0-1.0>`
Set volume in config.json. Validate range 0.0 to 1.0.

### `/anime-fx visuals <on|off>`
Toggle terminal visual animations (sparkles, beams, etc.) independent of sound.

### `/anime-fx on` / `/anime-fx off`
...
TTS ships **disabled by default** — users must run `/anime-fx tts setup` (or manually provide an API key + enable) before hearing anything.

### `/anime-fx tts setup`
First-run onboarding. Walk the user through enabling TTS:
1. Check if `ELEVENLABS_API_KEY` is set (env var or `.env`). If missing, prompt: "Paste your ElevenLabs API key (get one at https://elevenlabs.io). I'll write it to `.env`." Then call `/anime-fx tts key <pasted>`.
2. Ask which voice they want. Show the list from `/anime-fx tts voice`. Default suggestion: `rei`.
...
5. Run `/anime-fx tts test` to verify.

### `/anime-fx tts key <api_key>`
...
### `/anime-fx tts volume <0.0-1.0>`
Set TTS-specific volume in config.json `tts.volume`.

### `/anime-fx tts model <model_id>`
...
### `/anime-fx tts test [text]`
Test TTS by running: `echo '<text>' | node ~/Coding/anime-fx-plugin/scripts/test-tts.js`
...
2. Unzip it
3. Then run: `node ~/Coding/anime-fx-plugin/scripts/setup-dbz.js /path/to/unzipped/folder`

### `/anime-fx add-theme <name>`
